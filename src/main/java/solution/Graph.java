...
public class Graph {
     ...
     */
    public boolean hasPath(int vertex1, int vertex2, boolean[] visited) {
        if (vertex1 == vertex2) {
            System.out.println("Two vertices are the same.");
        ...
        while (!stack.empty()) {
            int v = stack.pop();
            visited[v] = true;
            Edge outgoingEdge = graph[v];
            while (outgoingEdge != null) {
                if (outgoingEdge.neighbor == vertex2)
                    return true;
                if (!visited[outgoingEdge.neighbor]) {
                    stack.push(outgoingEdge.neighbor);
                }
                outgoingEdge = outgoingEdge.next;
            }
        }
        return false;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(g.hasPath(7, 4, visited)); // true
        visited = new boolean[g.graph.length];
        System.out.println(g.hasPath(3, 6, visited)); // false
        visited = new boolean[g.graph.length];
        System.out.println(g.hasPath(1, 6, visited)); // true
    }
}
